...
<script>
    export default {
        props: ['elevator', 'buildings'],
        name: "Elevators-Edit-Component.vue",
        data() {
            return {
                newElevator: {
                    'name': this.elevator.name,
                    'index': this.elevator.index,
        ...
        methods: {
            createElevator: function createElevator() {
                ...
                if (input['name'] == '' || input['index'] <= 0 || input['building_id'] <= 0) {
                    ...
                    axios.put('/elevators/' + this.elevator.id, input).then(function (response) {
